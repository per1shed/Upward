...
class ChartRenderer:
    ...
    def _place_labels(
        ...
        """Place H:MM just above bars. Prefer overlap over stacking upward.

        Grid / scale lines and nearby columns may be overlapped — that is fine.
        """
        t = self.theme
        ax.set_ylim(0, y_top)
        if not labels:
        ...
        # Tallest bars first so their labels stay above overlapping neighbours
        items = sorted(labels, key=lambda r: (-r[1], r[0]))
        time_gap = max(y_top * 0.016, 0.1)
        star_gap = max(y_top * 0.045, 0.35)
